...
# error heat map
pAccml0=heatmap(yy, xx, log10.(diffs), xlabel=L"x^0",ylabel=L"\dot{x}^0")
...
# error heat map
pAccml0q1q2=heatmap(xx, xx, log10.(diffs), xlabel=L"x^0",ylabel=L"x^1")
...
pcontour_var_ml_el = heatmap(yy,xx,log10.(var_ml_el_val_q1q1dot),xlabel=L"x^0",ylabel=L"\dot{x}^0")
...
pcontour_var_ml_el = heatmap(xx,xx,log10.(var_ml_el_val_q1q2),xlabel=L"x^0",ylabel=L"x^1")
plot!(size=(200,133),right_margin=3Plots.mm);
...
plot(tt,pr(1).(sol_ref_interp), label="ref", color=:gray)
pCompareDyn=plot!(tt,pr(1).(sol_ml_interp), xlabel=L"t",ylabel=L"x^0", label="ml")
plot!(size=(300,200)); savefig(pCompareDyn,"plots/pCompareDyn_"*mNow()*".pdf")
